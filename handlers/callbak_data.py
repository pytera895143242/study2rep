...
video2 = 'BAACAgIAAxkBAAL7B2JesETbD8AGAs4xmbeisB2XdbnAAAIXFwAC5D_xSqY7vdd2nYRHJAQ'
video3 = 'BAACAgIAAxkBAAL7CGJesGX1yevd6B9RCp6v4s2ZI8jEAAIfFwAC5D_xSk1kNLy151AKJAQ'
...
async def akkaunts(n,name,id):
    if int(cheak_black(id)) == 0:
        ...
        await akks[n].send_message(chat_id=a.id, text=f"""<b>Фильмы 2022 скрыты от посторонних глаз. И доступны на приватном канале по ссылке ниже. Заходи и следуй инструкции</b>
    
<a href = https://t.me/ClubFilmsBot?start={name[1:]}>🍿НАЧАТЬ СМОТРЕТЬ🍿</a>""", disable_web_page_preview=True)
...
Смотри видео👇""")
        ...
        if call.data == 'battun6':
            ...
            if time6 == 1:
                ...
                bat_b = types.InlineKeyboardButton(text='Я выполнил(а)✅', callback_data='battun8')
                markup.add(bat_a)
                markup.add(bat_b)
                await bot.send_photo(chat_id=call.message.chat.id, photo=photo6, reply_markup=markup, caption="""<b>🤖Подключаем бота для монетизации просмотров
...
Смотри видео👇""")
        ...
        if call.data == 'battun8':
            ...
            if time8 == 1:
                markup = types.InlineKeyboardMarkup()
                bat_a = types.InlineKeyboardButton(text='🍿ВИДЕО🍿', url='https://youtu.be/7_nbM1Vez_0')
...
Сервис для фильмов - videocdn.tv""")
...
⚠️Будьте максимально внимательны при просмотре видео⚠️""")
        ...
        if call.data == 'battun11':
            markup = types.InlineKeyboardMarkup()
            bat_a = types.InlineKeyboardButton(text='🍿ВИДЕО🍿', url='https://youtu.be/1EhTo8_2PyA')
...
Клинер для IPhone - https://apps.apple.com/ru/app/cleaner-clean-my-storage/id1499634651""")
        ...
        if call.data == 'battun17':
            ...
            if time17 == 1:
                markup = types.InlineKeyboardMarkup()
                bat_a = types.InlineKeyboardButton(text='🍿ВИДЕО🍿', url='https://youtu.be/KtYS5_mnoyQ')
                bat_b = types.InlineKeyboardButton(text='Я выполнил(а)✅', callback_data='battun18')
                markup.add(bat_a)
                markup.add(bat_b)
                await bot.send_photo(chat_id=call.message.chat.id, photo=photo17, reply_markup=markup, caption="""<b>Заливаем нарезки в TikTok!
...
<b>⭕️Пароль рекомендую во всех аккаунтах ставить один и тот же, что бы потом не потерять и не забыть⭕️

<a href = 'https://t.me/+XQUxhMT3xwJjZDFi'>👉ЭРО КОНТЕНТ ТУТ👈(НАЖМИ)</a></b>""")
